...
describe("<Controls />", () => {
  ...
  it("should display disabled 'open gate' button after locking gate with props", () => {
    const { getByText } = render(<Controls closed={true} locked={true} />);
    expect(getByText(/open gate/i)).toBeDisabled();
  });

  it("should display enabled 'unlock gate' button after locking gate with props", () => {
    const { getByText } = render(<Controls closed={true} locked={true} />);
    expect(getByText(/unlock gate/i)).toBeEnabled();
  });

  it("should display enabled 'open gate' and 'lock gate' buttons after unlocking gate with props", () => {
    const { getByText } = render(<Controls closed={true} locked={false} />);
    expect(getByText(/lock gate/i)).toBeEnabled();
    expect(getByText(/open gate/i)).toBeEnabled();
  });

  it("should display enabled 'close gate' and disabled 'lock gate' buttons after opening gate with props", () => {
